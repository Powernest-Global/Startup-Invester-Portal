...
const ActivityCard = ({ item }) => (
  <div className="p-5 rounded-xl bg-[#1A1C23] border border-gray-800 flex items-center space-x-4 hover:bg-[#222630] transition">
    {item.type === "person" ? (
      <img
        src={item.avatar}
        alt={item.name}
    ...
    <div className="flex-1">
      ...
      <p className="text-gray-300 text-sm mt-1">{item.activity}</p>
    </div>
  ...
  <div className="py-3 border-b border-gray-800 last:border-0">
    <div className="flex justify-between items-start">
      <div>
        ...
        <p className="text-gray-400 text-xs mt-0.5">{meeting.time}</p>
      </div>
        ...
        {meeting.attendees
          ...
          .map((attendee, i) => (
            <img
              ...
              src={attendee}
              alt="attendee"
              className="w-6 h-6 rounded-full border-2 border-[#1A1C23]"
            />
          ))}
        {meeting.attendees.length > MAX_ATTENDEES_SHOWN && (
          <div className="w-6 h-6 rounded-full bg-gray-600 flex items-center justify-center text-white text-[10px] font-semibold border-2 border-[#1A1C23]">
            +{meeting.attendees.length - MAX_ATTENDEES_SHOWN}
          </div>
        )}
      </div>
  ...
  return (
    <main className="flex-1 p-8 bg-gray min-h-screen text-white">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <div className="inline-block px-3 py-1 rounded-full bg-green-600 text-xs font-semibold mb-2">
            Active
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="flex justify-between items-center mb-4">
            ...
            >
              View all <ChevronRight size={16} className="text-blue-400" />
            </a>
          </div>
        ...
        <div className="bg-[#1A1C23] rounded-xl border border-gray-800 p-6 h-fit">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-white">Meetings</h2>
            <div className="flex space-x-1 text-sm p-1 bg-[#0B0E11] rounded-full">
          ...
          <p className="text-gray-400 text-sm mb-3">Today, 30 July 2025</p>
          <div>
